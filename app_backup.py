...
def upload():
    ...
    # Add scan to blockchain
    blockchain.add_block({
        "action": "Image Scan",
        "filename": filename,
        "timestamp": datetime.now().isoformat(),
        "detections": detections,
        "total_threats": len(detections) if isinstance(detections, list) else 0
    })
...
def blockchain_page():
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>⛓ Blockchain Ledger - X-RAY Scanner</title>
        <style>
            :root {
                --primary: #00d4ff;
                --primary-dark: #0088cc;
    ...
    <body>
        <div class="container">
            <header>
                ...
                <p class="subtitle">Immutable record of all X-RAY scans and transactions</p>
            </header>
            
            <div class="nav">
                <a href="/">← Back to Scanner</a>
                <a href="/validate">Validate Chain</a>
            </div>
            ...
            <div class="stats">
                ...
                <div class="stat-box">
                    ...
                    <div class="stat-label">Scan Transactions</div>
                </div>
                <div class="stat-box">
                    <div class="stat-value">{% if chain %}{{ chain[-1].hash[:16] }}...{% endif %}</div>
            ...
            <div class="chain-container">
                {% for block in chain %}
                <div class="block {% if block.index == 0 %}genesis-block{% endif %}">
                    ...
                    <div class="block-data">
                        ...
                        <div class="data-item">
                            ...
                            <span class="data-value">{{ block.data.action }}</span>
                        </div>
                        {% endif %}
                        ...
                        <div class="data-item">
                            ...
                            <span class="data-value">{{ block.data.filename }}</span>
                        </div>
                        {% endif %}
                        
                        {% if block.data.total_threats is defined %}
                        <div class="data-item">
                            <span class="data-label">Threats Detected:</span>
                            <span class="data-value">{{ block.data.total_threats }}</span>
                        </div>
                        {% endif %}
                        
                        {% if block.data.detections %}
                        <div class="detections-list">
                            <div class="data-label">Detection Details:</div>
                            {% for detection in block.data.detections %}
                            <div class="detection-item">
                                <strong>{{ detection.item }}</strong> - {{ detection.severity }} ({{ detection.confidence }}%)
                            </div>
                            {% endfor %}
                        </div>
                        {% endif %}
                        ...
                        {% if key not in ['action', 'filename', 'total_threats', 'detections', 'timestamp'] %}
                        <div class="data-item">
    ...
    """
    return render_template_string(blockchain_html, chain=blockchain.chain)
