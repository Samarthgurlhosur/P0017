...
# allow importing P0017 from sibling folder when running SSS- app
...
p0017_module = None
try:
    p0017_dir = os.path.join(parent_dir, "P0017")
    if p0017_dir not in sys.path:
        sys.path.insert(0, p0017_dir)
    p0017_path = os.path.join(p0017_dir, "app.py")
    spec = importlib.util.spec_from_file_location("p0017_module", p0017_path)
    if spec and spec.loader:
        p0017_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(p0017_module)
except Exception:
    p0017_module = None
...
users = {
    ...
    "security": {"password": "sec123", "role": "security"}
}
    ...
    <body>
            ...
            <div class="card" role="main">
                <div class="left">
                    ...
                    <div class="roles" role="tablist" aria-label="Select role">
                        ...
                        <button type="button" id="securityBtn" class="role-btn" onclick="selectRole('security','security','sec123')">Security</button>
                    </div>

                    <div class="hint" id="roleHint">Select a role to auto-fill demo credentials.</div>
                    <div class="demo-note">Demo accounts: gov / gov123 • contractor / contract123 • public / public123 • security / sec123</div>
                </div>
        ...
        <script>
            const roleMap = {
                ...
                "security": {user: "security", pass: "sec123", btnId: "securityBtn"}
            };
...
def p0017_portal():
    if session.get("role") != "security":
        return redirect("/")
    if not p0017_module:
        return "P0017 module unavailable"
    return p0017_module.index()


@app.route("/blockchain")
def p0017_blockchain():
    if session.get("role") != "security":
        return redirect("/")
    if not p0017_module:
        return "P0017 module unavailable"
    return p0017_module.blockchain_page()


@app.route("/upload", methods=["POST"])
def p0017_upload():
    if not p0017_module:
        return jsonify({"success": False, "error": "P0017 module unavailable"}), 500
    return p0017_module.upload()
...
def p0017_detect():
    if not p0017_module:
        return jsonify({"success": False, "error": "P0017 module unavailable"}), 500
    return p0017_module.detect()


@app.route("/detect_dual", methods=["POST"])
def p0017_detect_dual():
    if not p0017_module:
        return jsonify({"success": False, "error": "P0017 module unavailable"}), 500
    return p0017_module.detect_dual()
...
if __name__ == "__main__":
    ...
    print("Starting Transparency Dashboard with SocketIO on http://127.0.0.1:5000")
    print("Waiting for connections...")
    try:
        socketio.run(app, host="127.0.0.1", port=5000, debug=False, allow_unsafe_werkzeug=True)
    except Exception as e:
        print(f"Error starting server: {e}")
        # Fallback to simple Flask
        print("Falling back to Flask without SocketIO...")
